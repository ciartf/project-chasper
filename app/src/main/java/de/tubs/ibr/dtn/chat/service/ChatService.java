...
import dataaccess.SensorMessageDataAccess;
...
import de.tubs.ibr.dtn.chat.LoginActivity;
...
import requestresponse.SubMessageRequest;
import requestresponse.SubMessageRequestBean;
import requestresponse.SubMessageResponse;
import requestresponse.TokenRequest;
import requestresponse.TokenResponse;
import tool.Tool;
import tool.WiFiConnection;

import static constant.GlobalData.subs;
...
public class ChatService extends DTNIntentService {
	...
	private DataHandler _data_handler = new DataHandler()
	{
		...
		private void eventNewMessage(SingletonEndpoint source, Date created, String payload, Long flags)
		{
            ...
            sensorMessage.setIs_sent("false");
            String keyword;
            String value;
            // search for the delimiter
            ...
            if (delimiter == -1) {
                ...
                // split the keyword and data pair
                keyword = payload.substring(0, delimiter);
                value = payload.substring(delimiter + 1, payload.length()).trim();
            }
            sensorMessage.setMessage(value);
            sensorMessage.setSensor_type(keyword);
            sensorMessage.setSource(source.toString());
            String uuid  = Tool.getUUID();
            sensorMessage.setUuid_sensor_message(uuid);
            SensorMessageDataAccess.addOrReplace(getApplicationContext(), sensorMessage);
	...
	private SharedPreferences.OnSharedPreferenceChangeListener mPrefListener = new SharedPreferences.OnSharedPreferenceChangeListener() {
		...
		public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
			...
			if ("presencetag".equals(key)) {
				...
				//adding from me
				//subtitute active and deactive using flag
				//true for doing presence broadcast, false for not doing that
...
//			}
			...
			//adding from me
			//upload pending data from local database, when AlarmManager active
			// and user is not logged as mqtt static relay (bypass user),
			//stop and start presence broadcast using flag
			if(!GeneralData.getUserId().equalsIgnoreCase(GlobalData.mqttUser)){
				uploadData();
			}
			if(GeneralData.getPresenceHackFlag()){
				SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(ChatService.this);
	...
	private void actionSendMessage(Long buddyId, String text) {
		try {
			...
			//my code
			//insert end point address directly to manipulate message destination
			//change buddy.getEndpoint() with string that represent end point address
	...
	private void uploadData(){
		try{
			if(LoginActivity.wiFiConnection.isConnected()){
				LoginActivity.wiFiConnection.disconnect();
				Thread.sleep(1000);
			}
			if(Tool.isInternetConnected(getApplicationContext())){
				List<SensorMessage> sensorMessageList = SensorMessageDataAccess.getAllIsNotSent(getApplicationContext());
				List<SubMessageRequestBean> subBeanList = new ArrayList<SubMessageRequestBean>();

				if(sensorMessageList != null){
					for(SensorMessage sensorMessage:sensorMessageList){
						if(Arrays.asList(GlobalData.sensorList).contains(sensorMessage.getSensor_type())){
							SubMessageRequestBean subBean = new SubMessageRequestBean();
							subBean.setSensor(sensorMessage.getSensor_type());
							subBean.setData(sensorMessage.getMessage());
							subBeanList.add(subBean);
						}else{
							SensorMessageDataAccess.delete(getApplicationContext(), sensorMessage);
							sensorMessageList.remove(sensorMessage);
						}
					}

					//TODO: request response subscription
					//marshall request now
					SubMessageRequest subRequest = new SubMessageRequest();
					subRequest.setSubMessageRequestBeanList(subBeanList);
					String jsonRequest = GsonHelper.toJson(subRequest);
					SubMessageResponse subResponse;
					String jsonResponse;
					HttpClient httpClient = new HttpClient();
					jsonResponse = httpClient.postMessage(jsonRequest, url+subs, GeneralData.getToken());
					if(jsonResponse != null){
						if(jsonResponse.equalsIgnoreCase("retryToken")){
							getToken();
							uploadData();
						}else{
							//unmarshall now
							//if needed
//							subResponse = GsonHelper.fromJson(jsonResponse, SubMessageResponse.class);
							for(SensorMessage sensorMessage:sensorMessageList){
								sensorMessage.setIs_sent("true");
							}
							SensorMessageDataAccess.addOrReplace(getApplicationContext(), sensorMessageList);
							Toast.makeText(getApplicationContext(), "Sending Message Success", Toast.LENGTH_LONG).show();
						}
					}else{
						Toast.makeText(getApplicationContext(), "Sending Message Error", Toast.LENGTH_LONG).show();
					}
				}
			}
			//adding from me
	...
	private void getToken(){
		...
		tokenRequest.setUser(GeneralData.getUserId());
		tokenRequest.setLabel(GlobalData.defaultLoginLabel);
		tokenRequest.setSecretkey(GeneralData.getPassword());
		...
		if(Tool.isInternetConnected(getApplicationContext())){
			try {
				...
				if(jsonResponse != null){
					...
					GeneralData.setToken("JWT "+tokenResponse.getToken());
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
